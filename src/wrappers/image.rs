...
pub fn load_hwc<T: AsRef<Path>>(path: T) -> Result<Tensor, TchError> {
    ...
    let c_tensor = unsafe_torch_err!(torch_sys_plus::at_load_image(path.as_ptr()));
    Ok(Tensor { c_tensor })
}
...
pub fn load_hwc_from_mem(data: &[u8]) -> Result<Tensor, TchError> {
    let c_tensor =
        unsafe_torch_err!(torch_sys_plus::at_load_image_from_memory(data.as_ptr(), data.len()));
    Ok(Tensor { c_tensor })
}
...
pub fn save_hwc<T: AsRef<Path>>(t: &Tensor, path: T) -> Result<(), TchError> {
    let path = path_to_cstring(path)?;
    let _ = unsafe_torch_err!(torch_sys_plus::at_save_image(t.c_tensor, path.as_ptr()));
    Ok(())
}
...
pub fn resize_hwc(t: &Tensor, out_w: i64, out_h: i64) -> Result<Tensor, TchError> {
    let c_tensor = unsafe_torch_err!({
        torch_sys_plus::at_resize_image(t.c_tensor, out_w as c_int, out_h as c_int)
    });
    Ok(Tensor { c_tensor })
}
