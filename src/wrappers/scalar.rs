...
pub struct Scalar {
    pub(super) c_scalar: *mut torch_sys_plus::C_scalar,
}

impl Scalar {
    /// Creates an integer scalar.
    pub fn int(v: i64) -> Scalar {
        let c_scalar = unsafe_torch!(torch_sys_plus::ats_int(v));
        Scalar { c_scalar }
    }

    /// Creates a float scalar scalar.
    pub fn float(v: f64) -> Scalar {
        let c_scalar = unsafe_torch!(torch_sys_plus::ats_float(v));
        Scalar { c_scalar }
    }

    /// Returns an integer value.
    pub fn to_int(&self) -> Result<i64, TchError> {
        let i = unsafe_torch_err!(torch_sys_plus::ats_to_int(self.c_scalar));
        Ok(i)
    }

    /// Returns a float value.
    pub fn to_float(&self) -> Result<f64, TchError> {
        let f = unsafe_torch_err!(torch_sys_plus::ats_to_float(self.c_scalar));
        Ok(f)
    }
    ...
    pub fn to_string(&self) -> Result<String, TchError> {
        let s = unsafe_torch_err!({
            super::utils::ptr_to_string(torch_sys_plus::ats_to_string(self.c_scalar))
        });
        match s {
...
impl Drop for Scalar {
    fn drop(&mut self) {
        unsafe_torch!(torch_sys_plus::ats_free(self.c_scalar))
    }
}
