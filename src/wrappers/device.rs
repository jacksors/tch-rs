...
impl Cuda {
    ...
    pub fn device_count() -> i64 {
        let res = unsafe_torch!(torch_sys_plus::cuda::atc_cuda_device_count());
        i64::from(res)
    }

    /// Returns true if at least one CUDA device is available.
    pub fn is_available() -> bool {
        unsafe_torch!(torch_sys_plus::cuda::atc_cuda_is_available()) != 0
    }

    /// Returns true if CUDA is available, and CuDNN is available.
    pub fn cudnn_is_available() -> bool {
        unsafe_torch!(torch_sys_plus::cuda::atc_cudnn_is_available()) != 0
    }
    ...
    pub fn manual_seed(seed: u64) {
        unsafe_torch!(torch_sys_plus::cuda::atc_manual_seed(seed));
    }
    ...
    pub fn manual_seed_all(seed: u64) {
        unsafe_torch!(torch_sys_plus::cuda::atc_manual_seed_all(seed));
    }
    ...
    pub fn synchronize(device_index: i64) {
        unsafe_torch!(torch_sys_plus::cuda::atc_synchronize(device_index));
    }
    ...
    pub fn user_enabled_cudnn() -> bool {
        unsafe_torch!(torch_sys_plus::cuda::atc_user_enabled_cudnn()) != 0
    }

    /// Enable or disable cudnn.
    pub fn set_user_enabled_cudnn(b: bool) {
        unsafe_torch!(torch_sys_plus::cuda::atc_set_user_enabled_cudnn(i32::from(b)))
    }
    ...
    pub fn cudnn_set_benchmark(b: bool) {
        unsafe_torch!(torch_sys_plus::cuda::atc_set_benchmark_cudnn(i32::from(b)))
    }
}
